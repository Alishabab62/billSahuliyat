...
const OwnerShipChangeForm = () => {
    ...
    const handleFormSubmit = () => {
        if(inputs.cCode === "" || inputs.sLoad === "" || inputs.rName === "" || inputs.mNumber === "" || inputs.address === "" || inputs.tCategory === "" || inputs.cChanged === ""){
            alert("Please enter all required fields");
            return;
        }
        window.location.href="/ownership-change-upload";
    }
  return (
    <div className="w-screen p-10 bg-[#F4F7FE]">
        <Header path={"Ownership Change"} name={"Request for Ownership Change"}/>
        <div className=' p-2'>
            <div className='w-full md:w-2/3 m-auto'>
                {/* <h3 className='font-bold text-3xl  mb-1 text-[#2B3674]'>Change of ownership</h3> */}
            ...
            <div className='w-full md:w-2/3 m-auto bg-white pt-4 pb-4 pl-7 pr-7 rounded-2xl'>
                <div >
                    ...
                    <div className='md:flex justify-between w-full'>
                        <div className='w-1/2'>
                            <label className='flex text-[#2B3674] text-xs'>Name of person in whose name connection to be changed <FaAsterisk className='text-red-500 text-[5px]'/></label>
                            <input type='text' onChange={handleInputs} name='cChanged' value={inputs.cChanged}   className='border p-2 my-2 rounded-xl block w-full'/>
                        </div>
                    </div>
                </div>
